...
description: "Inspect and operate the installed WaniKani for Omarchy client: study status, learning views, explicit study starts, lookup, and recovery guidance through its CLI. Use for the learner's desktop client, not plugin development or general WaniKani API work."
---
...
- Activity history is explicitly recorded on this device and can retain work from before resets. The CLI can open Activity; it does not currently export its detailed history. Do not bypass that boundary by reading SQLite.
