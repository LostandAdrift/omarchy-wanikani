...
The listening scenario generates three valid one-second MP3 tones locally with `ffmpeg`. They are transport fixtures, not Japanese speech. Only the copied QA Panel uses `AudioOutput.volume: 0`; production audio preferences are untouched. The copied worker rejects both account and media network requests. The scenario checks default autoplay after an explicit Start/Rate, manual Replay without a second exposure, an answer-free front and accessibility projection, reveal, local ratings, silent close/resume, and recap. A read-only comparison of the marked fixture database verifies that graded sessions, references and submissions are unchanged by listening. Decoder status, duration and playback-event counts remain in the capture snapshots.

Lesson discovery checks type filters, explicit selection and recommended preview without starting graded work. Study rhythm checks that editing local times changes only a draft until Apply. All runtime helpers must be tracked or staged before preparing the copy. `activate` and `focus` selectors may use a stable `objectName`; exact text selectors must include visible counts when present.
