...
Version **0.2.3**, frozen tree `a3896729ee5c7cc6e78596f2f07ef734c1f62112` in `/tmp/wanikani-milestone-5-release`, passed **873 tests in253.952s**; log `/tmp/wanikani-milestone-5-release-tests.log`. Manifest validation, all113 core Qt checks and archive identity checks passed (218 tracked files). All35 runtime QML files linted with zero errors and481 warnings. These include an existing layout-managed Study progress-bar height warning, without an established visual failure; this is not a warning-free build.
