...
- 489 distinct Python tests verified, including real subprocess interruption checks across persistence and submission boundaries. 112 core Qt checks passed, with additional component/lifecycle wrappers. Manifest and both installed desktop launchers validate; Hyprland has no configuration errors. See VERIFICATION.md for the final combined run and late focused regressions.
...
The sixth batch adds personal notes in lesson/mistake views, accessible similar-kanji comparisons, a compact selectable forecast, quiet recall, and responsive narrow panel chrome. It finishes upcoming offline-readiness access checks, native radical decoding/fallback guards, hidden-page/audio work cancellation, and lossless command-journal replies. Verified coverage is 468 full-suite Python tests plus one late comparison-render wrapper, 107 core Qt checks, five Forecast surface checks, 34 hosted native captures, and 5,000 additional randomized interactions. No new performance claim was made; the user redirected the remaining work toward UI and creative learning features. See the 05:06 UTC checkpoint in VERIFICATION.md.


The seventh batch adds a local reading trail to selection lookup, preserving the original passage while opening catalogue matches and returning to the same text. It includes theme-aware links, compact word disclosure, pending/uncertain labels, protected paused-study links, whole-code-point limits and stale-navigation guards. Verified coverage is 489 distinct Python tests and 112 core Qt checks; the native scenario now includes 36 captures. See the latest evidence in VERIFICATION.md for the precise run boundaries and remaining live-account/daily-use qualification.

The final runtime checkpoint `38a902e` is installed and running. After the shell restart, the original production demo session matched its pre-update digest, exactly one worker was running, and no temporary QA plugin remained. The final native 36-capture run and installed namespace had no warnings or errors. The practical [fourteen-day qualification template](DAILY_USE.md) records the first live sessions and subsequent daily use. The [next feature proposals](NEXT_FEATURES.md) specify practice from the reading trail, contrast practice and a visual lesson path; these extensions remain proposals.


First user-authorized live onboarding exposed and fixed a user-envelope schema error: the API's account UUID is at `data.id`. Runtime `ddff7e1` uses one validated identity in authentication, keyring, sync and milestones; 104 related tests passed. The copied token connected successfully, initial read synchronization completed, and the normal shell worker restored it from Secret Service. No live graded submissions were made. See the first live authentication section in VERIFICATION.md; prior four-hour notes above describe fixture-only work.
